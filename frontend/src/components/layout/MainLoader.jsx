...
const RemakeLogo = ({
  ...
  color = "black",
  transform = "",
  repeatDelay = 2, // Reduced from 3 to 2 seconds for faster animation
  mask = "",
}) => {
  const [key, setKey] = useState(0)

  // Animation loop control - faster cycle
  useEffect(() => {
    const interval = setInterval(() => {
      setKey((prevKey) => prevKey + 1)
    }, repeatDelay * 800) // Reduced from 1000ms to 800ms per second
  ...
  const containerVariants = {
    ...
    visible: {
      opacity: 1,
      transition: {
        staggerChildren: 0.075, // Reduced from 0.125 to 0.075
  ...
  const childVariants = {
    ...
    visible: {
      ...
      transition: {
        type: "spring",
        stiffness: 300, // Increased from 260 to 300
        damping: 18, // Reduced from 20 to 18
      },
    },
    exit: {
      opacity: 0,
  ...
  // Staggered letter paths (unchanged)
  const letterPaths = [
    <motion.path
      key="y"
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        <g transform={transform}>{letterPaths}</g>
      </motion.svg>
    </AnimatePresence>
  )
}
...
const MainLoader = ({ initialProgress = null }) => {
  ...
  const animationFrameRef = useRef(null) // Added ref for animation frame

  // Memoize viewport dimensions
  const width = typeof window !== "undefined" ? window.innerWidth : 1000
  const height = typeof window !== "undefined" ? window.innerHeight : 800

  // Optimize spring configuration for faster movement
  const smoothLoadingPercent = useSpring(0, {
    stiffness: 120, // Increased from 100 to 120
    damping: 25, // Decreased from 30 to 25
    restDelta: 0.001,
  })

  const waveHeightTransform = useTransform(smoothLoadingPercent, [0, 100], [height * 1.1, (height * 1.1 - height) * -1])

  // Memoized wave path generator for better performance
  const generateWavePath = (amplitude, frequency, phase, verticalSpeed, yOffset) => {
    ...
    // Increased step size from 5 to 10 for faster rendering
    for (let x = -100; x <= extendedWidth; x += 10) {
  ...
  // Optimized wave configurations
  const waves = [
    { amplitude: 20, frequency: 1, phase: 0, opacity: 0.8, yOffset: 0, verticalSpeed: 2.2, color: "#FFFFFF" }, // Increased vertical speed
    { amplitude: 18, frequency: 0.75, phase: 1, opacity: 0.6, yOffset: 15, verticalSpeed: 2.0, color: "#FFFFFF" }, // Increased vertical speed
    { amplitude: 15, frequency: 1.25, phase: 0.5, opacity: 0.5, yOffset: 30, verticalSpeed: 1.8, color: "#FFFFFF" }, // Increased vertical speed
  ]
    ...
    const updateLoadingPercent = () => {
      setLoadingPercent((prevPercent) => {
        if (prevPercent < 100) {
          ...
          const increment = 30 // Increased from 20 to 30
          ...
          if (newPercent < 100) {
            ...
            timerRef.current = gsap.delayedCall(0.03, updateLoadingPercent) // Reduced from 0.05 to 0.03
          }
  ...
  // Handle wave - optimized animation step
  useEffect(() => {
    if (!isInitialized) return

    const animate = () => {
      // Larger time step for faster wave movement
      setTime((prevTime) => prevTime + 0.04) // Increased from 0.025 to 0.04
